...
import { clearBookingData } from "../../../redux/cabbookingSlice";
import "react-toastify/dist/ReactToastify.css";
import axios from "axios";

const CabPaymentPage = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const bookingData = useSelector((state) => state.cabBooking.bookingData);
  ...
  const cabName = bookingData.cabName || "Toyota Innova";
  const pickup = bookingData.pickup || {};
  const contact = bookingData.contact || {};
  const baseFare =
    bookingData.selectedPackage?.offerPrice ?? bookingData.selectedPackage?.price ?? 3500;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        cabName: bookingData.cabName,
        packageType: bookingData.packageType,
        cabpackage: bookingData.selectedPackage?.title || "",
        ...
        pickup,
        contact,
        cabType: bookingData.cabType || "oneWay",
        bookingValue: totalFare,
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-4 border border-blue-100 space-y-4">
          ...
          <div className="text-sm text-gray-600 space-y-2">
            <p>
              <span className="font-medium">Cab:</span> {cabName}
            </p>
            <p>
              <span className="font-medium">Pickup City:</span> {pickup.city || "N/A"}
            </p>
            <p>
